...
function am(a,d2){
  ...
  const w=av.map(x=>{if(x.hl&&!x.ch)return a.chp<a.mhp*0.5?30:5;if(x.ch)return a.chp<a.mhp*0.8?15:5;if(x.gdl)return a.dlt>0?0:15;if(x.sw)return a.slt?5:18;if(x.gw)return a.wb===0?22:12;if(x.gg)return!terrain?25:3;if(x.ve)return d2.ps>=4?d2.ps*8:2;if(x.sb)return 12;let avg=x.f+x.dc[0]*(x.dc[1]+1)/2;if(x.dh)avg*=1.8;if(x.cbo&&a.dtt)avg*=1.75;if(x.rqF)avg*=1.3;if(x.bm)avg*=1.4;return avg;});
...
function eh(a,d2,ab){
  ...
  // Chivalry: goes last, heals if not damaged
...
function sim(c1,c2){
  ...
  for(let t=0;t<100;t++){
    ...
    // Resolve Chivalry: heals if not damaged this turn
...
const N=3000;
console.log('=== WARRIOR v3 (BT blocks RS, Chivalry=no dmg taken, last prio) ===\n');
