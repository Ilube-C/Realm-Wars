...
function clrEff(b){
  // Restoration: heal 20% on switch-out
...
function eh(a,d2,ab){
  ...
  if(ab.tr){if(a.status){d2.status=a.status;a.status=null;}if(a.ps>0){d2.ps+=a.ps;a.ps=0;}if(a.brn>0){d2.brn+=a.brn;a.brn=0;}if(a.bs>0){d2.bs+=a.bs;a.bs=0;}if(a.ws>0){d2.ws+=a.ws;a.ws=0;}ab.cu--;rs2();return;}
...
function buildTeam(){const pool=[...CLS];const team=[];const shuf=[...pool].sort(()=>Math.random()-0.5);for(let i=0;i<3;i++)team.push(cr(shuf[i]));return{members:team,active:null};}
function sim3v3(){
  terrain=null;const t1=buildTeam(),t2=buildTeam();
  ...
  for(let turn=0;turn<150;turn++){
    ...
    if(d1.a==='switch'){clrEff(a);t1.active=d1.t;apS(t1.active,rndS(t1.active));if(t1.active.stn?.p==='fb')b.bs+=2;sw1=true;}else{am1=am(a,b);}
    if(d2.a==='switch'){clrEff(b);t2.active=d2.t;apS(t2.active,rndS(t2.active));if(t2.active.stn?.p==='fb')t1.active.bs+=2;sw2=true;}else{am2=am(b,a);}
    const fa=t1.active,fb=t2.active;
    if(sw1&&sw2){tk(fa);tk(fb);}
    else if(sw1){eh(fb,fa,am2);tk(fa);tk(fb);}
    else if(sw2){eh(fa,fb,am1);tk(fa);tk(fb);}
    else{
      if(!am1&&!am2)break;
      ...
      // Ancient Power resolve for all team members
        ...
        for(let i=fighter.apq.length-1;i>=0;i--){
          ...
          if(fighter.apq[i].tl<=0){
            ...
            if(ap.tgt.chp>0&&!ap.tgt.fainted&&!fighter.fainted){
              const raw=14+rollDice(2,6);const m=0.5+0.5*(rollDice(2,6)+fighter.stats.int)/(rollDice(2,6)+ap.tgt.stats.int);
...
let draws=0;
for(let i=0;i<N;i++){
  const r=sim3v3();if(r.w===0){draws++;continue;}
...
console.log(`\n=== 3v3 TEAM BATTLE (7 chars, ${N} games, ${draws} draws) ===\n`);
...
for(const[comp,wins]of sorted)console.log(`  ${comp.padEnd(35)} ${wins} wins`);
