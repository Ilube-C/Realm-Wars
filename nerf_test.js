// Quick Mage nerf tester — runs 1v1 and 2v2 at different nerf levels
...
function generateStats(cls) {
  const w = cls.statWeights; const stats = {}; STAT_NAMES.forEach(s => stats[s] = STAT_FLOOR);
  let pool = STAT_BUDGET - 30; const tw = STAT_NAMES.reduce((a, s) => a + (w[s] || 1), 0);
  while (pool > 0) { let r = Math.random() * tw; for (const s of STAT_NAMES) { r -= (w[s] || 1); if (r <= 0) { if (stats[s] < STAT_CAP) { stats[s]++; pool--; } break; } } }
  return stats;
}

function makeAbilities(blastNerf, glacNerf, drainNerf, shatterNerf) {
  return {
    lichBlast:    { name:'Lich Blast', dmgType:'magical', stat:'int', fixed:8-blastNerf, dice:[2,6], uses:14, freezeChance:0.2 },
    glaciate:     { name:'Glaciate', dmgType:'magical', stat:'int', fixed:5-glacNerf, dice:[1,4], uses:16, freezeChance:0.6 },
    lichLifeDrain:{ name:'Life Drain', dmgType:'soul', stat:'int', fixed:7-drainNerf, dice:[1,8], uses:10, drain:0.5 },
    shatter:      { name:'Shatter', dmgType:'soul', stat:'int', fixed:18-shatterNerf, dice:[2,6], uses:6, requiresFrozen:true },
...
const CLASSES = [
  { id:'sirShining', label:'Paladin', statWeights:{atk:3,spd:1,con:3,int:1.5,def:4,cha:3.5}, abilities:['radialStrike','makeWay','heavenlyBlow','healingPrayer'],
    stances:[{id:'battlefieldStar',statBoosts:{cha:2},passive:'blindPerTurn'},{id:'flashyArrival',statBoosts:{def:2},passive:'flashyBlind'}]},
  { id:'pitDweller', label:'Berserker', statWeights:{atk:4,spd:1.5,con:3,int:0.5,def:2,cha:1}, abilities:['tumpUp','counterThrow','subdue','deathLust'],
    stances:[{id:'dirtyBoxing',statBoosts:{con:2},passive:'stunOnHit'},{id:'pitVeteran',statBoosts:{def:2},passive:'regenTick'}]},
  { id:'lich', label:'Mage', statWeights:{atk:0.5,spd:1.5,con:1.5,int:5,def:2.5,cha:3}, abilities:['lichBlast','glaciate','lichLifeDrain','shatter'],
    stances:[{id:'arcticAura',statBoosts:{con:2,cha:2},passive:'freezeAll'},{id:'soulSnatcher',statBoosts:{int:3},passive:'attackLifesteal'}]},
  { id:'rexRang', label:'Ranger', statWeights:{atk:4.5,spd:3,con:3,int:0.5,def:2,cha:1}, abilities:['battlerang','emberang','whittle','swerve'],
...
const level = 8;
function createCombatant(cls, AB) {
  const stats = generateStats(cls);
  const hp = 20 + stats.con * level * 0.5;
  return { cls, name:cls.label, stats, baseStats:{...stats}, maxHp:hp, currentHp:hp, level,
    abilities: cls.abilities.map(id => ({...AB[id], id, currentUses:AB[id].uses})),
    stances:cls.stances, stance:null, status:null, statusTurns:0, fainted:false,
    switchLocked:false, deathLustTurns:0, damagedThisTurn:false,
    blindStacks:0, boomerangHits:[], burnStacks:0, swerveActive:false, swerveLastTurn:false, whittleBoost:0, flinched:false };
}
...
function calcDmg(ab,atk,def){const raw=ab.fixed+rollDice(ab.dice[0],ab.dice[1]);if(ab.dmgType==='soul')return raw;if(ab.heal)return 0;if(ab.splitDamage){const pm=0.5+0.5*(rollDice(2,6)+atk.stats.atk)/(rollDice(2,6)+def.stats.def);const mm=0.5+0.5*(rollDice(2,6)+atk.stats.int)/(rollDice(2,6)+def.stats.int);return Math.max(1,Math.round(raw/2*pm+raw/2*mm));}const as=atk.stats[ab.stat]||atk.stats.atk;const ds=ab.dmgType==='physical'?def.stats.def:def.stats.int;return Math.max(1,Math.round(raw*(0.5+0.5*(rollDice(2,6)+as)/(rollDice(2,6)+ds))));}
function aiStance(b){return b.stances[Math.floor(Math.random()*b.stances.length)];}
function aiMove(a,d){const av=a.abilities.filter(x=>{if(x.currentUses<=0)return false;if(x.requiresFrozen&&d.status!=='frozen')return false;if(x.makeWay)return false;if(x.grantWhittle&&a.whittleBoost>=8)return false;return true;});if(!av.length)return null;const w=av.map(x=>{if(x.heal)return a.currentHp<a.maxHp*0.5?30:5;if(x.grantDeathLust)return a.deathLustTurns>0?0:15;if(x.swerve)return a.swerveLastTurn?5:18;if(x.grantWhittle)return a.whittleBoost===0?22:12;let avg=x.fixed+x.dice[0]*(x.dice[1]+1)/2;if(x.doubleHit)avg*=1.8;if(x.counterBonus&&a.damagedThisTurn)avg*=1.75;if(x.requiresFrozen)avg*=1.3;if(x.boomerang)avg*=1.4;return avg;});const t=w.reduce((s,v)=>s+v,0);let r=Math.random()*t;for(let i=0;i<av.length;i++){r-=w[i];if(r<=0)return av[i];}return av[av.length-1];}
...
function executeHit(atk,def,ab){
  ...
  if(ab.heal&&ab.healingPrayer){atk.currentHp=Math.min(atk.maxHp,atk.currentHp+Math.max(5,Math.round(atk.maxHp*0.25)));atk.status=null;atk.statusTurns=0;atk.blindStacks=0;atk.burnStacks=0;ab.currentUses--;rs();return;}
  ...
  for(let h=0;h<(ab.doubleHit?2:1);h++){
    ...
    if(h>0&&ab.doubleHit&&Math.random()<0.3)continue;
    if(def.swerveActive&&!ab.heal&&!ab.swerve&&!ab.grantWhittle){if(Math.random()<(def.swerveActive==='high'?1:0.2))continue;}
    ...
    if(atk.stance?.passive==='stunOnHit'&&!def.status&&def.currentHp>0&&Math.random()<0.2+Math.max(0,(atk.stats.cha-def.stats.cha))*0.02){def.status='stun';def.statusTurns=1;}
    ...
    if(atk.stance?.passive==='blindPerTurn'&&def.currentHp>0&&Math.random()<0.15+Math.max(0,(atk.stats.cha-def.stats.cha))*0.02)def.blindStacks++;
    if(ab.burnChance&&def.currentHp>0&&Math.random()<ab.burnChance+Math.max(0,(atk.stats.cha-def.stats.cha))*0.02)def.burnStacks++;
    if(ab.boomerang&&def.currentHp>0){const bm=atk.stance?.passive==='boomerangBonus'?1.3:1.0;def.boomerangHits.push({damage:Math.round(dmg*bm)});}
    if(h===0&&ab.flinchChance&&def.currentHp>0&&!def.flinched){if(Math.random()<ab.flinchChance+Math.max(0,(atk.stats.cha-def.stats.cha))*0.02)def.flinched=true;}
  }
  rs();
}
...
function tick(b){
  ...
  b.switchLocked=false;b.swerveLastTurn=!!b.swerveActive;b.swerveActive=false;b.flinched=false;
  ...
  if(b.boomerangHits.length>0&&b.currentHp>0){for(const h of b.boomerangHits.splice(0)){b.currentHp=Math.max(0,b.currentHp-h.damage);b.damagedThisTurn=true;if(b.currentHp<=0)break;}}
  if(b.status==='stun'){b.status=null;b.statusTurns=0;}
  if(b.defBonusThisTurn){b.stats.def-=b.defBonusThisTurn;b.defBonusThisTurn=0;}
}

function sim1v1(cls1,cls2,AB){
  const a=createCombatant(cls1,AB),b=createCombatant(cls2,AB);
  ...
  for(let t=0;t<100;t++){
    ...
    tick(a);tick(b);if(a.currentHp<=0||b.currentHp<=0)break;
  }
  if(a.currentHp>0&&b.currentHp<=0)return cls1.id;if(b.currentHp>0&&a.currentHp<=0)return cls2.id;return'draw';
}

// Test different nerf levels
const N = 3000;
const nerfs = [
  {label:'Current', blast:0, glac:0, drain:0, shatter:0},
  {label:'Nerf 1 (Blast-1, Drain-1)', blast:1, glac:0, drain:1, shatter:0},
  {label:'Nerf 2 (Blast-2, Drain-1)', blast:2, glac:0, drain:1, shatter:0},
  {label:'Nerf 3 (Blast-2, Drain-1, Shatter-2)', blast:2, glac:0, drain:1, shatter:2},
  {label:'Nerf 4 (Blast-1, Glac-1, Drain-1)', blast:1, glac:1, drain:1, shatter:0},
];

const mage = CLASSES[2];
const others = CLASSES.filter(c => c.id !== 'lich');

for (const nerf of nerfs) {
  const AB = makeAbilities(nerf.blast, nerf.glac, nerf.drain, nerf.shatter);
  console.log(`\n=== ${nerf.label} ===`);
  console.log(`  Blast:${8-nerf.blast} Glac:${5-nerf.glac} Drain:${7-nerf.drain} Shatter:${18-nerf.shatter}`);
  for (const opp of others) {
    const res = {[mage.id]:0,[opp.id]:0,draw:0};
    for(let i=0;i<N;i++)res[sim1v1(mage,opp,AB)]++;
    console.log(`  Mage vs ${opp.label}: ${(res[mage.id]/N*100).toFixed(1)}% | ${(res[opp.id]/N*100).toFixed(1)}% | ${(res.draw/N*100).toFixed(1)}% draw`);
  }
}
