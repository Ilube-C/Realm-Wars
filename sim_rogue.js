// Rogue (Count Coction) balance testing sim
...
const AB={
  lichBlast:{name:'Lich Blast',dmgType:'magical',stat:'int',fixed:7,dice:[2,6],uses:14,freezeChance:0.2},
  glaciate:{name:'Glaciate',dmgType:'magical',stat:'int',fixed:5,dice:[1,4],uses:16,freezeChance:0.6},
  lichLifeDrain:{name:'Life Drain',dmgType:'soul',stat:'int',fixed:6,dice:[1,8],uses:10,drain:0.5},
  ...
  poisonDart:{name:'Poison Dart',dmgType:'physical',stat:'atk',fixed:6,dice:[1,6],uses:14,poisonChance:0.40},
  goblinGas:{name:'Goblin Gas',dmgType:'physical',stat:'atk',fixed:0,dice:[1,1],uses:4,goblinGas:true},
  remedialOintment:{name:'Remedial Ointment',dmgType:'heal',stat:'cha',fixed:0,dice:[1,1],uses:6,remedialOintment:true,heal:true},
  violentExtraction:{name:'Violent Extraction',dmgType:'soul',stat:'atk',fixed:0,dice:[1,1],uses:6,violentExtraction:true},
};
const CLASSES=[
  ...
  {id:'countCoction',label:'Rogue',statWeights:{atk:4,spd:5,con:1.5,int:0.5,def:1,cha:2},abilities:['poisonDart','goblinGas','remedialOintment','violentExtraction'],
   stances:[{id:'rapidTransmission',statBoosts:{cha:2},passive:'rapidTransmission'},{id:'neurotoxin',statBoosts:{spd:2},passive:'neurotoxin'}]},
];
const level=8;
let terrain=null;

function create(cls){
  const stats=genStats(cls);const hp=20+stats.con*level*0.5;
  return{cls,name:cls.label,stats,baseStats:{...stats},maxHp:hp,currentHp:hp,level,
    ...
    blindStacks:0,boomerangHits:[],burnStacks:0,swerveActive:false,swerveLastTurn:false,whittleBoost:0,flinched:false,poisonStacks:0};
}
...
function calcDmg(ab,atk,def,oppOfDef){
  const raw=ab.fixed+rollDice(ab.dice[0],ab.dice[1]);
  if(ab.dmgType==='soul')return raw;if(ab.heal)return 0;
  if(ab.splitDamage){return Math.max(1,Math.round(raw/2*(0.5+0.5*(rollDice(2,6)+atk.stats.atk)/(rollDice(2,6)+def.stats.def))+raw/2*(0.5+0.5*(rollDice(2,6)+atk.stats.int)/(rollDice(2,6)+def.stats.int))));}
  let as=atk.stats[ab.stat]||atk.stats.atk;
  let ds=ab.dmgType==='physical'?def.stats.def:def.stats.int;
  // Neurotoxin
  if(oppOfDef&&oppOfDef.stance?.passive==='neurotoxin'&&def.poisonStacks>0&&ab.dmgType==='magical')ds=Math.max(1,ds-def.poisonStacks);
  if(oppOfDef&&oppOfDef.stance?.passive==='neurotoxin'&&atk.poisonStacks>0&&ab.stat==='int')as=Math.max(1,as-atk.poisonStacks);
...
function aiMove(a,d){
  ...
  // Remedial when poisoned and hurt
  ...
  const av=a.abilities.filter(x=>{
    if(x.currentUses<=0)return false;if(x.requiresFrozen&&d.status!=='frozen')return false;
    if(x.makeWay)return false;if(x.grantWhittle&&a.whittleBoost>=8)return false;
    if(x.remedialOintment)return false; // handled above
    return true;
  });
  if(!av.length)return null;
  const w=av.map(x=>{
    if(x.heal)return a.currentHp<a.maxHp*0.5?30:5;
    if(x.grantDeathLust)return a.deathLustTurns>0?0:15;
    if(x.swerve)return a.swerveLastTurn?5:18;
    if(x.grantWhittle)return a.whittleBoost===0?22:12;
    if(x.goblinGas)return !terrain?25:3;
    if(x.violentExtraction)return d.poisonStacks>=3?d.poisonStacks*8:2;
    let avg=x.fixed+x.dice[0]*(x.dice[1]+1)/2;
    if(x.doubleHit)avg*=1.8;if(x.counterBonus&&a.damagedThisTurn)avg*=1.75;
    if(x.requiresFrozen)avg*=1.3;if(x.boomerang)avg*=1.4;
    return avg;
  });
  const t=w.reduce((s,v)=>s+v,0);let r=Math.random()*t;
  for(let i=0;i<av.length;i++){r-=w[i];if(r<=0)return av[i];}return av[av.length-1];
}

function applyPoison(target,stacks,source){
  // Rapid Transmission check
  if(source.stance?.passive==='rapidTransmission'){
    const rtChance=0.30+Math.max(0,(source.stats.cha-target.stats.cha))*0.02;
    if(Math.random()<rtChance)stacks*=2;
  }
...
function executeHit(atk,def,ab){
  ...
  if(ab.remedialOintment){const st=atk.poisonStacks;if(st>0){atk.poisonStacks=0;atk.currentHp=Math.min(atk.maxHp,atk.currentHp+st*5);}ab.currentUses--;rs();return;}
  if(ab.violentExtraction){const st=def.poisonStacks;if(st>0){def.poisonStacks=0;def.currentHp-=st*6;def.damagedThisTurn=true;}ab.currentUses--;rs();return;}
  ab.currentUses--;
  for(let h=0;h<(ab.doubleHit?2:1);h++){
    ...
    let dmg=calcDmg(ab,atk,def,atk);if(h>0&&ab.doubleHit)dmg=Math.round(dmg*1.5);
...
function tick(b){
  ...
  if(b.poisonStacks>0&&b.currentHp>0){b.currentHp=Math.max(0,b.currentHp-b.poisonStacks*2);}
...
function sim1v1(cls1,cls2){
  terrain=null;
  const a=create(cls1),b=create(cls2);
  ...
  for(let t=0;t<100;t++){
    ...
    if(terrain&&terrain.turnsLeft>0){
      ...
      if(terrain.effect==='poisonBoth'){
        for(const fighter of[a,b]){if(fighter.currentHp>0){const enemy=fighter===a?b:a;applyPoison(fighter,1,enemy);}}
      }
      if(terrain.turnsLeft<=0)terrain=null;
    }
...
// Run 1v1 matchups — Rogue vs everyone
const N=3000;
const rogue=CLASSES.find(c=>c.id==='countCoction');
console.log('=== ROGUE 1v1 MATCHUPS ===');
for(const opp of CLASSES){
  if(opp.id===rogue.id)continue;
  const res={[rogue.id]:0,[opp.id]:0,draw:0};
  for(let i=0;i<N;i++)res[sim1v1(rogue,opp)]++;
  console.log(`Rogue vs ${opp.label}: Rogue ${(res[rogue.id]/N*100).toFixed(1)}% | ${opp.label} ${(res[opp.id]/N*100).toFixed(1)}% | ${(res.draw/N*100).toFixed(1)}% draw`);
}
...
for(let i=0;i<CLASSES.length;i++){
  for(let j=i+1;j<CLASSES.length;j++){
    ...
    const res={[c1.id]:0,[c2.id]:0,draw:0};
    for(let k=0;k<N;k++)res[sim1v1(c1,c2)]++;
    console.log(`${c1.label} vs ${c2.label}: ${(res[c1.id]/N*100).toFixed(1)}% | ${(res[c2.id]/N*100).toFixed(1)}% | ${(res.draw/N*100).toFixed(1)}% draw`);
  }
}
