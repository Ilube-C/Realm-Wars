'use strict';
// Smart player (optimal decisions) vs Standard AI (random stance, weighted moves, new switching)
// Tests whether a skilled player can beat the AI consistently
...
const AB={
  ...
  lichLifeDrain:{t:'m',s:'int',f:7,dc:[1,8],u:10,dr:0.4},shatter:{t:'s',s:'int',f:18,dc:[2,6],u:6,rqF:1},
...
function matchScore(me,them){let s=0;if(me.cls.sw.atk>=3&&them.stats.def<10)s+=15;if(me.cls.sw.int>=3&&them.stats.int<10)s+=15;if(me.cls.id==='paladin'&&them.cls.id==='rogue')s+=20;if(me.cls.id==='mage'&&them.cls.id==='paladin')s+=15;if(me.cls.id==='warrior'&&them.cls.id==='rogue')s+=15;if(me.cls.id==='rogue'&&them.cls.id==='mage')s+=5;if(me.cls.id==='warrior'&&them.cls.id==='druid')s+=15;if(me.cls.id==='druid'&&them.cls.id==='rogue')s+=15;if(me.cls.id==='ranger'&&them.cls.id==='paladin')s+=10;if(me.cls.id==='ranger'&&them.cls.id==='berserker')s+=10;if(me.cls.id==='rogue'&&them.cls.id==='berserker')s+=10;if(me.cls.id==='rogue'&&them.cls.id==='cleric')s+=20;if(me.cls.id==='cleric')s-=10;s+=(me.chp/me.mhp-them.chp/them.mhp)*20;return s;}

// SMART player: picks best stance for matchup, optimal switches, best moves
function smartPickStance(me, enemy) {
  // Evaluate each stance by simulating a few rounds mentally
  let best = me.st[0], bestScore = -999;
  for (const st of me.st) {
    let score = 0;
    // Stat boosts value
    if (st.sb) for (const [k, v] of Object.entries(st.sb)) {
      if (k === 'atk' && me.cls.sw.atk >= 3) score += v * 3;
      if (k === 'int' && me.cls.sw.int >= 3) score += v * 3;
      if (k === 'def') score += v * 2;
      if (k === 'cha') score += v * 1.5;
      if (k === 'con') score += v * 2;
      if (k === 'spd') score += v * 1;
    }
    // Passive value based on matchup
    if (st.p === 'fa' && enemy.cls.sw.def >= 3) score += 10; // Freeze vs tanky
    if (st.p === 'al' && me.cls.sw.int >= 3) score += 8; // Lifesteal for sustained
    if (st.p === 'soh' && enemy.cls.sw.spd >= 3) score += 8; // Stun vs fast
    if (st.p === 'rt' && me.cls.sw.con >= 3) score += 10; // Regen on tanky chars
    if (st.p === 'bpt') score += 5; // Blind is always useful
    if (st.p === 'fb' && enemy.cls.sw.atk >= 3) score += 8; // DEF+blind vs physical
    if (st.p === 'bb' && me.cls.sw.atk >= 4) score += 8; // Boomerang boost
    if (st.p === 'ov' && enemy.stn) score += 8; // Suppress enemy passive
    if (st.p === 'cpt') score += 7; // Curse pressure
    if (st.p === 'hb' && me.cls.id === 'cleric') score += 8; // Better heals/shields
    if (st.p === 'rest') score += 6; // Switch heal
    if (score > bestScore) { bestScore = score; best = st; }
  }
  return best;
}

function smartDecision(active, bench, enemy) {
  if (active.swl || active.lookAtMeLocked) return { a: 'fight' };
  const ab2 = bench.filter(b => !b.fainted && b !== active);
  if (!ab2.length) return { a: 'fight' };
  const cs = matchScore(active, enemy);
  let bs2 = null, bsc = cs;
  for (const b of ab2) {
    const sc = matchScore(b, enemy) + (b.chp / b.mhp) * 10;
    if (sc > bsc) { bsc = sc; bs2 = b; }
  }
  // Smart player is more aggressive about switching
  if (active.chp < active.mhp * 0.30 && bs2 && bsc > cs + 5) return { a: 'switch', t: bs2 };
  if (bs2 && bsc > cs + 20) return { a: 'switch', t: bs2 };
  return { a: 'fight' };
}

// AI decision (standard — same thresholds as live game)
function aiDecision(active, bench, enemy) {
  if (active.swl || active.lookAtMeLocked) return { a: 'fight' };
  const ab2 = bench.filter(b => !b.fainted && b !== active);
  if (!ab2.length) return { a: 'fight' };
  const cs = matchScore(active, enemy);
...
// Smart move selection (slightly better than standard AI)
function smartMove(a, d2, myTeam) {
  if (a.chp < a.mhp * 0.25) { const hi = a.ab.findIndex(x => x.hl && !x.ch && x.cu > 0); if (hi >= 0) return a.ab[hi]; }
  if (a.ps >= 2 && a.chp < a.mhp * 0.5) { const ri = a.ab.findIndex(x => x.ro && x.cu > 0); if (ri >= 0) return a.ab[ri]; }
  // Prioritize VE at 3+ stacks (lower threshold)
  if (d2.ps >= 3) { const vi = a.ab.findIndex(x => x.ve && x.cu > 0); if (vi >= 0) return a.ab[vi]; }
  ...
  const w2 = av.map(x => {
    if (x.hl && !x.ch) return a.chp < a.mhp * 0.4 ? 35 : 3;
    if (x.ch) return a.chp < a.mhp * 0.7 ? 18 : 3;
    if (x.gdl) return a.dlt > 0 ? 0 : 18;
    if (x.sw) return a.slt ? 3 : 22;
    if (x.gw) return a.wb === 0 ? 25 : 15;
    if (x.gg) return !terrain ? 28 : 2;
    if (x.ve) return d2.ps >= 3 ? d2.ps * 10 : 1;
    if (x.sb) return 12;
    if (x.ev) return d2.ws < 3 ? 20 : 12;
    if (x.ap) return 24;
    if (x.tr) return (a.ps + a.brn + a.bs + a.ws + a.cs) * 8 + (a.status ? 18 : 0);
    if (x.ml) { const hurt = myTeam ? myTeam.filter(m => !m.fainted && m.chp < m.mhp * 0.8).length : 0; return hurt > 0 ? hurt * 14 : a.chp < a.mhp * 0.6 ? 22 : 2; }
    if (x.rs2) return 16;
    if (x.ri) return 18;
    if (x.lam) return 12;
    let avg = x.f + x.dc[0] * (x.dc[1] + 1) / 2;
    if (x.dh) avg *= 1.8;
    if (x.cbo && a.dtt) avg *= 1.75;
    if (x.rqF) avg *= 1.5;
    if (x.bm) avg *= 1.5;
    return avg;
  });
  const t2 = w2.reduce((s, v) => s + v, 0); let r = Math.random() * t2;
  for (let i = 0; i < av.length; i++) { r -= w2[i]; if (r <= 0) return av[i]; }
  return av[av.length - 1];
}

// Standard AI move (same as live game)
function aiMove(a, d2, myTeam) {
  ...
  const w2 = av.map(x => {
    ...
    if (x.ml) { const hurt = myTeam ? myTeam.filter(m => !m.fainted && m.chp < m.mhp * 0.8).length : 0; return hurt > 0 ? hurt * 12 : a.chp < a.mhp * 0.7 ? 20 : 3; }
    if (x.rs2) return 15;
    if (x.ri) return 16;
    if (x.lam) return 10;
    let avg = x.f + x.dc[0] * (x.dc[1] + 1) / 2;
...
function eh(a,d2,ab){
  ...
  if(ab.ml){const heal=Math.max(8,Math.floor(a.mhp*0.25))*(a.stn?.p==='hb'?1.3:1);a.chp=Math.min(a.mhp,a.chp+Math.floor(heal));a.mlHeal=Math.floor(heal);ab.cu--;rs();return;}
  if(ab.lam){d2.lookAtMeLocked=true;ab.cu--;rs();return;}
...
function mlHeal(team,si){for(const m of team){if(m!==si&&m.mlHeal>0){si.chp=Math.min(si.mhp,si.chp+m.mlHeal);m.mlHeal=0;break;}}}

const N = 100;
let smartWins = 0, aiWins = 0, draws = 0;

for (let i = 0; i < N; i++) {
  terrain = null;
  const pool = [...CLS].sort(() => Math.random() - 0.5);
  // Smart player gets to pick first (slight advantage — knows the matchup)
  const t1 = { members: pool.slice(0, 4).map(c => cr(c)), active: null };
  const t2 = { members: pool.slice(4, 8).map(c => cr(c)), active: null };
  t1.active = t1.members[0]; t2.active = t2.members[0];

  // Smart player picks stance based on matchup
  apS(t1.active, smartPickStance(t1.active, t2.active));
  // AI picks random stance
  apS(t2.active, rndS(t2.active));
  
  if (t1.active.stn?.p === 'fb') t2.active.bs += 2;
  if (t2.active.stn?.p === 'fb') t1.active.bs += 2;
  ...
  for (let turn = 0; turn < 200; turn++) {
    ...
    // Smart player decision
    const d1 = smartDecision(a, t1.members, b);
    // AI decision
    const d2 = aiDecision(b, t2.members, a);
    
    let am1 = null, am2 = null, sw1 = false, sw2 = false;
    
    if (d1.a === 'switch') {
      clrEff(a); t1.active = d1.t;
      apS(t1.active, smartPickStance(t1.active, t2.active));
      if (t1.active.stn?.p === 'fb') b.bs += 2;
      mlHeal(t1.members, t1.active); sw1 = true;
    } else { am1 = smartMove(a, b, t1.members); }
    
    if (d2.a === 'switch') {
      clrEff(b); t2.active = d2.t;
      apS(t2.active, rndS(t2.active));
      if (t2.active.stn?.p === 'fb') t1.active.bs += 2;
      mlHeal(t2.members, t2.active); sw2 = true;
    } else { am2 = aiMove(b, a, t2.members); }
    
    const fa = t1.active, fb = t2.active;
    if (sw1 && sw2) { tk(fa, fb); tk(fb, fa); }
    else if (sw1) { eh(fb, fa, am2); tk(fa, fb); tk(fb, fa); }
    else if (sw2) { eh(fa, fb, am1); tk(fa, fb); tk(fb, fa); }
    else {
      if (!am1 && !am2) break;
      ...
      const [f, s, fm, sm] = ai2 >= bi2 ? [fa, fb, am1, am2] : [fb, fa, am2, am1];
      ...
      for (const fighter of [...t1.members, ...t2.members]) {
        ...
        for (let j = fighter.apq.length - 1; j >= 0; j--) {
          fighter.apq[j].tl--;
          if (fighter.apq[j].tl <= 0) {
            const ap = fighter.apq.splice(j, 1)[0];
            if (ap.tgt.chp > 0 && !ap.tgt.fainted && !fighter.fainted) {
              const raw = 15 + rollDice(2, 6); const m = 0.5 + 0.5 * (rollDice(2, 6) + fighter.stats.int) / (rollDice(2, 6) + ap.tgt.stats.int);
    ...
    if (t1.active.fainted) {
      const al = t1.members.filter(m => !m.fainted);
      if (!al.length) { winner = 2; break; }
      let best = al[0], bs2 = -999;
      for (const b2 of al) { const sc = matchScore(b2, t2.active); if (sc > bs2) { bs2 = sc; best = b2; } }
      t1.active = best; apS(t1.active, smartPickStance(t1.active, t2.active));
      if (t1.active.stn?.p === 'fb') t2.active.bs += 2; mlHeal(t1.members, t1.active);
    }
    if (t2.active.fainted) {
      const al = t2.members.filter(m => !m.fainted);
      if (!al.length) { winner = 1; break; }
      let best = al[0], bs2 = -999;
      for (const b2 of al) { const sc = matchScore(b2, t1.active); if (sc > bs2) { bs2 = sc; best = b2; } }
      t2.active = best; apS(t2.active, rndS(t2.active));
      if (t2.active.stn?.p === 'fb') t1.active.bs += 2; mlHeal(t2.members, t2.active);
    }
  }
  if (winner === 1) smartWins++;
  else if (winner === 2) aiWins++;
  else draws++;
}

console.log(`\n=== SMART PLAYER vs STANDARD AI (${N} games) ===`);
console.log(`Smart wins: ${smartWins} (${(smartWins/N*100).toFixed(1)}%)`);
console.log(`AI wins:    ${aiWins} (${(aiWins/N*100).toFixed(1)}%)`);
console.log(`Draws:      ${draws}`);
console.log(`\nSmart advantages: better stance picks, lower switch thresholds, slightly better move weights`);
