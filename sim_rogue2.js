// Rogue tuning — test multiple buff levels
...
function makeAB(dartFixed,dartPoison,extractDmg,poisonDmgPerStack,ointmentHeal){
  return{
    ...
    poisonDart:{name:'Poison Dart',dmgType:'physical',stat:'atk',fixed:dartFixed,dice:[1,6],uses:14,poisonChance:dartPoison},
    goblinGas:{name:'Goblin Gas',dmgType:'physical',stat:'atk',fixed:0,dice:[1,1],uses:4,goblinGas:true},
    remedialOintment:{name:'Remedial Ointment',dmgType:'heal',stat:'cha',fixed:0,dice:[1,1],uses:6,remedialOintment:true,heal:true,ointmentHeal:ointmentHeal},
    violentExtraction:{name:'Violent Extraction',dmgType:'soul',stat:'atk',fixed:0,dice:[1,1],uses:6,violentExtraction:true,extractDmg:extractDmg},
  };
}

function makeCLS(conWeight){
  return[
    ...
    {id:'countCoction',label:'Rogue',statWeights:{atk:4,spd:5,con:conWeight,int:0.5,def:1,cha:2},abilities:['poisonDart','goblinGas','remedialOintment','violentExtraction'],stances:[{id:'rapidTransmission',statBoosts:{cha:2},passive:'rapidTransmission'},{id:'neurotoxin',statBoosts:{spd:2},passive:'neurotoxin'}]},
  ];
}
...
function applyPoison(target,stacks,source){if(source.stance?.passive==='rapidTransmission'){if(Math.random()<0.30+Math.max(0,(source.stats.cha-target.stats.cha))*0.02)stacks*=2;}target.poisonStacks+=stacks;}
function aiMove(a,d,AB){
...
function sim1v1(cls1,cls2,AB){
  ...
  for(let t=0;t<100;t++){
    ...
    const am=aiMove(a,b,AB),bm=aiMove(b,a,AB);if(!am&&!bm)break;
...
const N=2000;
const configs=[
  {label:'Buff 1: Dart 8+1d8, 55% poison, Extract 8/stack, CON 2.5',dartFixed:8,dice:8,dartPoison:0.55,extractDmg:8,poisonDmg:2,ointmentHeal:5,conWeight:2.5},
  {label:'Buff 2: Dart 9+1d8, 50% poison, Extract 7/stack, CON 3',dartFixed:9,dice:8,dartPoison:0.50,extractDmg:7,poisonDmg:3,ointmentHeal:6,conWeight:3},
  {label:'Buff 3: Dart 8+1d6, 60% poison, Extract 8/stack, Poison 3/stack, CON 2.5',dartFixed:8,dice:6,dartPoison:0.60,extractDmg:8,poisonDmg:3,ointmentHeal:6,conWeight:2.5},
];

for(const cfg of configs){
  const AB=makeAB(cfg.dartFixed,cfg.dartPoison,cfg.extractDmg,cfg.poisonDmg,cfg.ointmentHeal);
  const CLS=makeCLS(cfg.conWeight);
  const rogue=CLS.find(c=>c.id==='countCoction');
  console.log(`\n=== ${cfg.label} ===`);
  for(const opp of CLS){
    if(opp.id===rogue.id)continue;
    const res={[rogue.id]:0,[opp.id]:0,draw:0};
    for(let i=0;i<N;i++)res[sim1v1(rogue,opp,AB)]++;
