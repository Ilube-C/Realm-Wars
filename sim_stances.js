'use strict';
// Stance usage sim — tracks win rate per stance choice
// Copy core from sim4v4.js but track stance picks
...
const CLS=[
  {id:'paladin',l:'Paladin',ab:['radialStrike','heavenlyBlow','healingPrayer','heavenlyBlow'],sw:{atk:3,spd:1,con:3,int:1.5,def:4,cha:3.5},
    st:[{id:'bstar',n:'Battlefield Star',sb:{cha:2},p:'bpt'},{id:'flashy',n:'Flashy Arrival',sb:{def:2},p:'fb'}]},
  {id:'berserker',l:'Berserker',ab:['tumpUp','counterThrow','subdue','deathLust'],sw:{atk:4,spd:1.5,con:3,int:0.5,def:2,cha:1},
    st:[{id:'dirty',n:'Dirty Boxing',sb:{con:2},p:'soh'},{id:'pitvet',n:'Pit Veteran',sb:{def:2},p:'rt'}]},
  {id:'mage',l:'Mage',ab:['lichBlast','glaciate','lichLifeDrain','shatter'],sw:{atk:0.5,spd:1.5,con:1.5,int:5,def:2.5,cha:3},
    st:[{id:'arctic',n:'Arctic Aura',sb:{con:2,cha:2},p:'fa'},{id:'soul',n:'Soul Snatcher',sb:{int:3},p:'al'}]},
  {id:'ranger',l:'Ranger',ab:['battlerang','emberang','whittle','swerve'],sw:{atk:4.5,spd:3,con:3,int:0.5,def:2,cha:1},
    st:[{id:'patient',n:'Patient Killer',sb:{atk:2},p:'bb'},{id:'guerilla',n:'Guerilla Fighter',sb:{spd:2},p:'gs'}]},
  {id:'rogue',l:'Rogue',ab:['poisonDart','goblinGas','remedialOintment','violentExtraction'],sw:{atk:3,spd:4,con:3,int:0.5,def:2,cha:1.5},
    st:[{id:'rapid',n:'Rapid Transmission',sb:{cha:2},p:'rpt'},{id:'night',n:'Night Terrors',sb:{spd:2},p:'nt'}]},
  {id:'warrior',l:'Warrior',ab:['recklessSwing','eviscerate','lexShieldBash','chivalry'],sw:{atk:4,spd:2,con:3.5,int:0.5,def:3,cha:1},
    st:[{id:'over',n:'Overwhelming',sb:{atk:3},p:'ov'},{id:'duel',n:'Duelist',sb:{def:2},p:'du'}]},
  {id:'druid',l:'Druid',ab:['petalStorm','ancientPower','transference','petalStorm'],sw:{atk:0.5,spd:1,con:1,int:3.5,def:1.5,cha:2},
    st:[{id:'overgrow',n:'Overgrowth',sb:{int:2},p:'og',ogE:1},{id:'restore',n:'Restoration',sb:{con:2},p:'rest'}]},
  {id:'cleric',l:'Cleric',ab:['moonlight','readScripture','riot','lookAtMe'],sw:{atk:0.5,spd:1,con:2,int:3.5,def:2,cha:3},
    st:[{id:'gekyume',n:"Gekyume's Blessing",sb:{int:2},p:'hb'},{id:'sermon',n:'Forbidden Sermon',sb:{cha:2},p:'cpt'}]},
];
let terrain=null;
...
// Track stance wins/picks
const stanceWins={}, stancePicks={};
for(const cls of CLS) for(const st of cls.st) {
  const key=`${cls.l}: ${st.n}`;
  stanceWins[key]=0; stancePicks[key]=0;
}
...
for(let i=0;i<N;i++){
  ...
  // Track initial stances
  const stanceLog=[];
  for(const team of[t1,t2]){
    for(const m of team.members){
      const st=rndS(m);apS(m,st);
      stanceLog.push({cls:m.cls.l,stance:st.n,team:team===t1?1:2});
      const key=`${m.cls.l}: ${st.n}`;stancePicks[key]++;
    }
  }
  ...
  for(let turn=0;turn<200;turn++){
    ...
    if(t1.active.fainted){const al=t1.members.filter(m=>!m.fainted);if(!al.length){winner=2;break;}let best=al[0],bs2=-999;for(const b2 of al){const sc=matchScore(b2,t2.active);if(sc>bs2){bs2=sc;best=b2;}}t1.active=best;const ns=rndS(t1.active);apS(t1.active,ns);const key=`${t1.active.cls.l}: ${ns.n}`;stancePicks[key]++;stanceLog.push({cls:t1.active.cls.l,stance:ns.n,team:1});if(t1.active.stn?.p==='fb')t2.active.bs+=2;mlHeal(t1.members,t1.active);}
    if(t2.active.fainted){const al=t2.members.filter(m=>!m.fainted);if(!al.length){winner=1;break;}let best=al[0],bs2=-999;for(const b2 of al){const sc=matchScore(b2,t1.active);if(sc>bs2){bs2=sc;best=b2;}}t2.active=best;const ns=rndS(t2.active);apS(t2.active,ns);const key=`${t2.active.cls.l}: ${ns.n}`;stancePicks[key]++;stanceLog.push({cls:t2.active.cls.l,stance:ns.n,team:2});if(t2.active.stn?.p==='fb')t1.active.bs+=2;mlHeal(t2.members,t2.active);}
  }
  if(winner>0){
    const winTeam=winner===1?1:2;
    for(const entry of stanceLog){
      if(entry.team===winTeam){
        const key=`${entry.cls}: ${entry.stance}`;stanceWins[key]++;
      }
    }
  }
}

console.log('\n=== STANCE WIN RATES (4v4, random pick, 3000 games) ===\n');
for(const cls of CLS){
  console.log(`${cls.l}:`);
  for(const st of cls.st){
    const key=`${cls.l}: ${st.n}`;
    const picks=stancePicks[key];
    const wins=stanceWins[key];
    const wr=picks>0?(wins/picks*100).toFixed(1):'0';
    const pctPick=((picks/(stancePicks[`${cls.l}: ${cls.st[0].n}`]+stancePicks[`${cls.l}: ${cls.st[1].n}`]))*100).toFixed(0);
    console.log(`  ${st.n.padEnd(22)} ${wr}% WR  (${picks} picks, ~${pctPick}%)`);
  }
}
