// Quick test: Mage CHA weight 3 vs 2 vs 1.5
'use strict';
...
function cs(b,c){return b+Math.max(0,c)*0.02}
// Paste the full sim inline but parameterize mage CHA weight
function runSim(mageChaW){
...
const CLS=[
  ...
  {id:'M',l:'Mage',sw:{atk:0.5,spd:1.5,con:1.5,int:5,def:2.5,cha:mageChaW},ab:['lichBlast','glaciate','lichLifeDrain','shatter'],st:[{sb:{con:2,cha:2},p:'fa'},{sb:{int:3},p:'al'}]},
...
function am(a,d){
  ...
  const av=a.ab.filter(x=>{if(x.cu<=0)return false;if(x.rqF&&d.status!=='f')return false;if(x.gw&&a.wb>=8)return false;if(x.ro)return false;return true;});
...
function eh(a,d,ab){
  ...
  if(ab.rqF&&d.status!=='f'){rs2();return;}
  ...
  if(ab.ve){const st=d.ps;if(st>0){d.ps=0;d.chp-=st*10;d.dtt=true;}ab.cu--;rs2();return;}
  ab.cu--;
  for(let h=0;h<(ab.dh?2:1);h++){
    ...
    if(d.status==='f'&&!ab.rqF){d.status=null;}
    if(ab.rqF&&d.status==='f'){d.status=null;}
    if(ab.dr)a.chp=Math.min(a.mhp,a.chp+Math.round(dmg*ab.dr));
    if(ab.sl)d.swl=true;
    if(a.stn?.p==='soh'&&!d.status&&d.chp>0&&Math.random()<cs(0.3,a.stats.cha-d.stats.cha)){d.status='s';}
    if(!d.status&&d.chp>0){let fc=ab.fc||0;if(a.stn?.p==='fa')fc+=0.1;fc=cs(fc,a.stats.cha-d.stats.cha);if(fc>0&&Math.random()<fc){d.status='f';}}
    if(ab.bc&&d.chp>0&&Math.random()<cs(ab.bc,a.stats.cha-d.stats.cha))d.bs++;
    if(a.stn?.p==='bpt'&&d.chp>0&&Math.random()<cs(0.25,a.stats.cha-d.stats.cha))d.bs++;
    if(ab.bn&&d.chp>0&&Math.random()<cs(ab.bn,a.stats.cha-d.stats.cha))d.brn++;
    if(ab.pc&&d.chp>0&&Math.random()<cs(ab.pc,a.stats.cha-d.stats.cha))aP(d,ab.pst||1,a);
    if(ab.bm&&d.chp>0){d.bh.push({dm:Math.round(dmg*(a.stn?.p==='bb'?1.3:1.0))});}
    if(h===0&&ab.fl&&d.chp>0&&!d.fl&&Math.random()<cs(ab.fl,a.stats.cha-d.stats.cha))d.fl=true;
  }
  rs2();
}
function tk(b){
...
const N=2000;
const rogue=CLS.find(c=>c.id==='Ro');
for(const opp of CLS){if(opp.id===rogue.id)continue;
  const res={Ro:0,[opp.id]:0,draw:0};for(let i=0;i<N;i++)res[sim(rogue,opp)]++;
  process.stdout.write(`  vs ${opp.l}: ${(res.Ro/N*100).toFixed(1)}/${(res[opp.id]/N*100).toFixed(1)}  `);
}
console.log();
// Also check Mage vs others
const mage=CLS.find(c=>c.id==='M');
for(const opp of CLS){if(opp.id===mage.id)continue;
  const res={M:0,[opp.id]:0,draw:0};for(let i=0;i<N;i++)res[sim(mage,opp)]++;
  process.stdout.write(`  M vs ${opp.l}: ${(res.M/N*100).toFixed(1)}/${(res[opp.id]/N*100).toFixed(1)}  `);
}
console.log();
return;
}
for(const chaW of [3, 2, 1.5, 1]){
  console.log(`\n=== Mage CHA weight: ${chaW} ===`);
  console.log('Rogue matchups:');
  runSim(chaW);
}
