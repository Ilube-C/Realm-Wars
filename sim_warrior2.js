...
const AB={
  ...
  recklessSwing:{t:'p',s:'atk',f:14,dc:[2,6],u:10,rs:1},bladeToss:{t:'p',s:'atk',f:7,dc:[1,6],u:12,bt:1},
  lexShieldBash:{t:'p',s:'atk',f:4,dc:[1,4],u:16,sb:1,sc:0.25},chivalry:{t:'h',s:'def',f:0,dc:[1,1],u:6,ch:1,hl:1},
};
const CLS=[
...
let terrain=null;
function cr(cls){const stats=gs(cls.sw);const hp=20+stats.con*4;return{cls,nm:cls.l,stats,mhp:hp,chp:hp,ab:cls.ab.map(id=>{const a=AB[id];return{...a,id,cu:a.u}}),st:cls.st,stn:null,status:null,dlt:0,dtt:false,bs:0,brn:0,bh:[],sa:false,slt:false,wb:0,fl:false,ps:0,swl:false,btc:false,ds:0,cp:false};}
...
function am(a,d2){
  ...
  const av=a.ab.filter(x=>{if(x.cu<=0)return false;if(x.rqF&&d2.status!=='f')return false;if(x.gw&&a.wb>=8)return false;if(x.ro)return false;if(a.btc&&x.bt)return false;return true;});
  if(!av.length)return null;
  const w=av.map(x=>{if(x.hl&&!x.ch)return a.chp<a.mhp*0.5?30:5;if(x.ch)return a.chp<a.mhp*0.7?12:5;if(x.gdl)return a.dlt>0?0:15;if(x.sw)return a.slt?5:18;if(x.gw)return a.wb===0?22:12;if(x.gg)return!terrain?25:3;if(x.ve)return d2.ps>=4?d2.ps*8:2;if(x.sb)return 12;let avg=x.f+x.dc[0]*(x.dc[1]+1)/2;if(x.dh)avg*=1.8;if(x.cbo&&a.dtt)avg*=1.75;if(x.rqF)avg*=1.3;if(x.bm)avg*=1.4;return avg;});
...
function eh(a,d2,ab,enemyAb){
  ...
  if(ab.ch){
    // Chivalry: resolve after both moves — check if enemy used non-damaging
    const nd=enemyAb&&(enemyAb.sw||enemyAb.gdl||enemyAb.gw||enemyAb.gg||enemyAb.hl||enemyAb.ch);
    if(nd){const heal=Math.max(10,Math.round(a.mhp*0.30));a.chp=Math.min(a.mhp,a.chp+heal);}
    ab.cu--;rs2();return;
  }
  let sbDef=0,sbSpd=0;
  ...
  for(let h=0;h<(ab.dh?2:1);h++){
    ...
    if(ab.rs&&dmg>0){const recoil=Math.max(1,Math.round(dmg*0.2));a.chp=Math.max(1,a.chp-recoil);}
  ...
  // Duelist: consume stacks after attacking (shield bash consumes too now)
...
function sim(c1,c2){
  ...
  for(let t=0;t<100;t++){
    ...
    eh(f,s,fm,sm);if(s.chp>0&&!s.fl)eh(s,f,sm,fm);f.fl=false;s.fl=false;
    ...
    tk(a);tk(b);
    const aBt=a2?.bt;const bBt=b2?.bt;if(!aBt)a.btc=false;if(!bBt)b.btc=false;
    if(a.chp<=0||b.chp<=0)break;
  }
  if(a.chp>0&&b.chp<=0)return c1.id;if(b.chp>0&&a.chp<=0)return c2.id;return'draw';
}
const N=3000;
console.log('=== WARRIOR v2 (20% recoil, BT only locks self, Chivalry on non-dmg) ===\n');
const w=CLS.find(c=>c.id==='W');
for(const opp of CLS){if(opp.id===w.id)continue;
  const res={W:0,[opp.id]:0,draw:0};for(let i=0;i<N;i++)res[sim(w,opp)]++;
  console.log(`Warrior vs ${opp.l.padEnd(10)}: ${(res.W/N*100).toFixed(1)}% | ${(res[opp.id]/N*100).toFixed(1)}%`);
}
console.log('\n--- Full matrix ---');
