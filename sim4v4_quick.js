'use strict';
// Quick test: Ranger/Rogue ATK buff + curse stats tracking
const fs=require('fs');
...
function runTest(label, rangerSw, rogueSw) {
  const CLS=[
    ...
    {id:'ranger',l:'Ranger',sw:rangerSw,ab:['battlerang','emberang','whittle','swerve'],st:[{sb:{atk:2},p:'bb'},{sb:{spd:2},p:'gs'}]},
    {id:'rogue',l:'Rogue',sw:rogueSw,ab:['poisonDart','goblinGas','remedialOintment','violentExtraction'],st:[{sb:{cha:2},p:'rpt'},{sb:{spd:2},p:'nt'}]},
  ...
  let terrain=null;
  // Track curse stats
  let curseApplied=0,curseTransformed=0,curseToFrozen=0,curseToStun=0,curseToBlind=0,curseToBurn=0,curseToPoison=0,curseToWound=0;
  ...
  function am2(a,d2,myTeam){if(a.chp<a.mhp*0.3){const hi=a.ab.findIndex(x=>x.hl&&!x.ch&&x.cu>0);if(hi>=0)return a.ab[hi];}if(a.ps>=2&&a.chp<a.mhp*0.6){const ri=a.ab.findIndex(x=>x.ro&&x.cu>0);if(ri>=0)return a.ab[ri];}const av=a.ab.filter(x=>{if(x.cu<=0)return false;if(x.rqF&&d2.status!=='f')return false;if(x.gw&&a.wb>=8)return false;if(x.ro)return false;return true;});if(!av.length)return null;const w2=av.map(x=>{if(x.hl&&!x.ch)return a.chp<a.mhp*0.5?30:5;if(x.ch)return a.chp<a.mhp*0.8?15:5;if(x.gdl)return a.dlt>0?0:15;if(x.sw)return a.slt?5:18;if(x.gw)return a.wb===0?22:12;if(x.gg)return!terrain?25:3;if(x.ve)return d2.ps>=4?d2.ps*8:2;if(x.sb)return 12;if(x.ev)return d2.ws<3?18:10;if(x.ap)return 22;if(x.tr)return(a.ps+a.brn+a.bs+a.ws+a.cs)*6+(a.status?15:0);if(x.ml){const hurt=myTeam?myTeam.filter(m=>!m.fainted&&m.chp<m.mhp*0.8).length:0;return hurt>0?hurt*12:a.chp<a.mhp*0.7?20:3;}if(x.rs2)return 15;if(x.ri)return 16;if(x.lam)return 10;let avg=x.f+x.dc[0]*(x.dc[1]+1)/2;if(x.dh)avg*=1.8;if(x.cbo&&a.dtt)avg*=1.75;if(x.rqF)avg*=1.3;if(x.bm)avg*=1.4;return avg;});const t2=w2.reduce((s2,v)=>s2+v,0);let r=Math.random()*t2;for(let i=0;i<av.length;i++){r-=w2[i];if(r<=0)return av[i];}return av[av.length-1];}
  function eh(a,d2,ab,myTeam,enemyTeam){
    ...
    if(ab.ri){d2.cs+=2;curseApplied+=2;d2.brn++;}
    if(ab.rs2){const r=Math.random();let cs=0;if(r<0.10)cs=3;else if(r<0.25)cs=2;else if(r<0.50)cs=1;if(cs>0){d2.cs+=cs;curseApplied+=cs;}}
    ...
    for(let h=0;h<(ab.dh?2:1);h++){
      ...
      if(d2.st2>0&&d2.shp>0&&dmg>0&&!ab.hl){const ab3=Math.min(dmg,d2.shp);d2.shp-=ab3;dmg-=ab3;if(d2.shp<=0)d2.st2=0;}
  ...
  function tk(b,enemy){
    ...
    if(b.cs>0&&b.chp>0){for(let i=0;i<b.cs;i++){if(Math.random()<0.35){const opts=[];if(b.status!=='f')opts.push('f');if(b.status!=='s')opts.push('s');opts.push('bl','bn','po','wo');const c=opts[Math.floor(Math.random()*opts.length)];b.cs--;curseTransformed++;if(c==='f'){b.status='f';curseToFrozen++;}else if(c==='s'){b.status='s';curseToStun++;}else if(c==='bl'){b.bs++;curseToBlind++;}else if(c==='bn'){b.brn++;curseToBurn++;}else if(c==='po'){b.ps++;curseToPoison++;}else if(c==='wo'){b.ws++;curseToWound++;}break;}}}
    if(b.stn?.p==='cpt'&&b.chp>0&&enemy&&enemy.chp>0){enemy.cs+=2;curseApplied+=2;}
  ...
  for(let i=0;i<N;i++){
    ...
    for(let turn=0;turn<200;turn++){
      ...
      let am1=null,_am2=null,sw1=false,sw2=false;
      if(d1.a==='switch'){if(a.lookAtMeLocked){const shT=b.stn?.p==='hb'?3:2;const shH=b.stn?.p==='hb'?20:15;for(const m of t2.members){if(!m.fainted){m.st2=Math.max(m.st2,shT);m.shp=Math.max(m.shp,shH);}}a.lookAtMeLocked=false;am1=am2(a,b,t1.members);}else{clrEff(a);t1.active=d1.t;apS(t1.active,rndS(t1.active));if(t1.active.stn?.p==='fb')b.bs+=2;mlHeal(t1.members,t1.active);sw1=true;}}else{am1=am2(a,b,t1.members);}
      if(d2.a==='switch'){if(b.lookAtMeLocked){const shT=a.stn?.p==='hb'?3:2;const shH=a.stn?.p==='hb'?20:15;for(const m of t1.members){if(!m.fainted){m.st2=Math.max(m.st2,shT);m.shp=Math.max(m.shp,shH);}}b.lookAtMeLocked=false;_am2=am2(b,a,t2.members);}else{clrEff(b);t2.active=d2.t;apS(t2.active,rndS(t2.active));if(t2.active.stn?.p==='fb')t1.active.bs+=2;mlHeal(t2.members,t2.active);sw2=true;}}else{_am2=am2(b,a,t2.members);}
      const fa=t1.active,fb=t2.active;
      if(sw1&&sw2){tk(fa,fb);tk(fb,fa);}
      else if(sw1){eh(fb,fa,_am2,t2.members,t1.members);tk(fa,fb);tk(fb,fa);}
      else if(sw2){eh(fa,fb,am1,t1.members,t2.members);tk(fa,fb);tk(fb,fa);}
      else{
        if(!am1&&!_am2)break;
        let ai2=gi(fa),bi2=gi(fb);
        if(am1?.cm)ai2=Math.floor(ai2*0.5);if(_am2?.cm)bi2=Math.floor(bi2*0.5);
        if(am1?.sw){ai2=Math.floor(ai2*1.5);fa.sa=!fa.slt?'high':'low';}
        if(_am2?.sw){bi2=Math.floor(bi2*1.5);fb.sa=!fb.slt?'high':'low';}
        if(am1?.ch)ai2=-100;if(_am2?.ch)bi2=-100;
        const[f,s,fm,sm,ft,st2]=ai2>=bi2?[fa,fb,am1,_am2,t1,t2]:[fb,fa,_am2,am1,t2,t1];
        ...
        for(const fighter of[...t1.members,...t2.members]){if(!fighter.apq)continue;for(let i=fighter.apq.length-1;i>=0;i--){fighter.apq[i].tl--;if(fighter.apq[i].tl<=0){const ap=fighter.apq.splice(i,1)[0];if(ap.tgt.chp>0&&!ap.tgt.fainted&&!fighter.fainted){const raw=15+rollDice(2,6);const m=0.5+0.5*(rollDice(2,6)+fighter.stats.int)/(rollDice(2,6)+ap.tgt.stats.int);const dmg=Math.max(1,Math.round(raw*m));ap.tgt.chp-=dmg;ap.tgt.dtt=true;}}}}
  ...
  const results=[];
  for(const cls of CLS){const p=classPicks[cls.l];results.push({cls:cls.l,wr:p>0?(classWins[cls.l]/p*100):0});}
  results.sort((a,b)=>b.wr-a.wr);
  const spread=results[0].wr-results[results.length-1].wr;
  console.log(`\n--- ${label} (spread: ${spread.toFixed(1)}%) ---`);
  for(const r of results)console.log(`  ${r.cls.padEnd(10)} ${r.wr.toFixed(1)}%`);
  if(curseApplied>0){
    console.log(`  Curse: ${curseApplied} applied, ${curseTransformed} transformed (${(curseTransformed/curseApplied*100).toFixed(0)}%)`);
    console.log(`    → Frozen:${curseToFrozen} Stun:${curseToStun} Blind:${curseToBlind} Burn:${curseToBurn} Poison:${curseToPoison} Wound:${curseToWound}`);
  }
}

// Current
runTest('Current', {atk:4.5,spd:3,con:3,int:0.5,def:2,cha:1}, {atk:3,spd:4,con:3,int:0.5,def:2,cha:1.5});
// Ranger ATK 4.5→5, Rogue ATK 3→3.5
runTest('Ran ATK 5, Rog ATK 3.5', {atk:5,spd:3,con:3,int:0.5,def:2,cha:1}, {atk:3.5,spd:4,con:3,int:0.5,def:2,cha:1.5});
// Ranger ATK 5, Rogue ATK 4
runTest('Ran ATK 5, Rog ATK 4', {atk:5,spd:3,con:3,int:0.5,def:2,cha:1}, {atk:4,spd:4,con:3,int:0.5,def:2,cha:1.5});
// Ranger ATK 5.5, Rogue ATK 4
runTest('Ran ATK 5.5, Rog ATK 4', {atk:5.5,spd:3,con:3,int:0.5,def:2,cha:1}, {atk:4,spd:4,con:3,int:0.5,def:2,cha:1.5});
