function d(n){return Math.floor(Math.random()*n)+1}function rollDice(c,s){let t=0;for(let i=0;i<c;i++)t+=d(s);return t}
...
// Track move usage per class across all battles
const moveUsage = {}; // class -> move -> {used, wonWith}
const stanceWins = {}; // class -> stance -> {used, wins}
...
function calcDmg(ab,a,d){const raw=ab.fixed+rollDice(ab.dice[0],ab.dice[1]);if(ab.dmgType==='soul')return raw;if(ab.heal)return 0;if(ab.splitDamage){return Math.max(1,Math.round(raw/2*(0.5+0.5*(rollDice(2,6)+a.stats.atk)/(rollDice(2,6)+d.stats.def))+raw/2*(0.5+0.5*(rollDice(2,6)+a.stats.int)/(rollDice(2,6)+d.stats.int))));}const as=a.stats[ab.stat]||a.stats.atk;const ds=ab.dmgType==='physical'?d.stats.def:d.stats.int;return Math.max(1,Math.round(raw*(0.5+0.5*(rollDice(2,6)+as)/(rollDice(2,6)+ds))));}
...
function executeHit(atk,def,ab){
  ...
  // Track usage
  atk.moveLog[ab.id||ab.name] = (atk.moveLog[ab.id||ab.name]||0) + 1;
...
const N=3000;
// Track per-stance win rates
for(const cls of CLASSES){
  console.log(`\n=== ${cls.label} ===`);
  
  // Stance analysis: force each stance and measure win rate
  for(const stance of cls.stances){
    let wins=0, total=0;
    for(const opp of CLASSES){
      if(opp.id===cls.id)continue;
      for(let i=0;i<N;i++){
        const a=create(cls),b=create(opp);
        applyStance(a,stance); // forced
        applyStance(b,aiStance(b));
        ...
        total++;if(a.currentHp>0&&b.currentHp<=0)wins++;
      }
    }
    console.log(`  ${stance.id}: ${(wins/total*100).toFixed(1)}% win rate`);
  }

  // Move usage: track across random battles
  const usage={};cls.abilities.forEach(id=>usage[id]={count:0,battles:0});
  for(const opp of CLASSES){
    if(opp.id===cls.id)continue;
    for(let i=0;i<1000;i++){
      ...
      for(const[mv,ct]of Object.entries(a.moveLog)){if(usage[mv])usage[mv].count+=ct;usage[mv]&&usage[mv].battles++;}
    }
  }
  console.log('  Move usage (avg per battle):');
  for(const[id,data]of Object.entries(usage)){
    const total_battles = 3000;
    console.log(`    ${AB[id]?.name||id}: ${(data.count/total_battles).toFixed(1)} uses/battle (used in ${(data.battles/total_battles*100).toFixed(0)}% of battles)`);
  }
}
