'use strict';
// Quick balance testing script — runs multiple configs and compares
const fs = require('fs');
...
function runSim(config) {
  const {physNerf, magBuff, soulBuff, cursePerTurn, critThreshold} = config;
  
  const AB={
    lichBlast:{t:'m',s:'int',f:Math.round(7*magBuff),dc:[2,6],u:14,fc:0.2},glaciate:{t:'m',s:'int',f:Math.round(5*magBuff),dc:[1,4],u:16,fc:0.6},
    lichLifeDrain:{t:'s',s:'int',f:Math.round(6*soulBuff),dc:[1,8],u:10,dr:0.5},shatter:{t:'s',s:'int',f:Math.round(18*soulBuff),dc:[2,6],u:6,rqF:1},
    tumpUp:{t:'p',s:'atk',f:Math.round(5*physNerf),dc:[1,6],u:16,dh:1},counterThrow:{t:'p',s:'atk',f:Math.round(7*physNerf),dc:[1,6],u:10,cm:1,cbo:0.75},
    subdue:{t:'p',s:'atk',f:Math.round(12*physNerf),dc:[1,6],u:12,sl:1},deathLust:{t:'p',s:'atk',f:6,dc:[1,4],u:4,gdl:1},
    radialStrike:{t:'p',s:'atk',f:Math.round(9*physNerf),dc:[1,6],u:16,bc:0.4},heavenlyBlow:{t:'p',s:'atk',f:Math.round(8*physNerf),dc:[1,8],u:10,sd:1},
    healingPrayer:{t:'h',s:'cha',f:0,dc:[1,1],u:4,hp:1,hl:1},
    battlerang:{t:'p',s:'atk',f:Math.round(8*physNerf),dc:[1,6],u:14,bm:1,fl:0.25},emberang:{t:'p',s:'atk',f:Math.round(5*physNerf),dc:[1,6],u:12,bm:1,bn:0.35},
    whittle:{t:'p',s:'atk',f:Math.round(4*physNerf),dc:[1,4],u:4,gw:1},swerve:{t:'p',s:'atk',f:0,dc:[1,1],u:10,sw:1},
    poisonDart:{t:'p',s:'atk',f:Math.round(8*physNerf),dc:[1,6],u:14,pc:0.60,pst:3},goblinGas:{t:'p',s:'atk',f:0,dc:[1,1],u:4,gg:1},
    remedialOintment:{t:'h',s:'cha',f:0,dc:[1,1],u:6,ro:1,hl:1},violentExtraction:{t:'s',s:'atk',f:0,dc:[1,1],u:6,ve:1},
    recklessSwing:{t:'p',s:'atk',f:Math.round(16*physNerf),dc:[2,6],u:10,rs:1},eviscerate:{t:'p',s:'atk',f:Math.round(8*physNerf),dc:[1,6],u:14,ev:1},
    lexShieldBash:{t:'p',s:'atk',f:Math.round(7*physNerf),dc:[1,4],u:16,sb:1,sc:0.35},chivalry:{t:'h',s:'def',f:0,dc:[1,1],u:6,ch:1,hl:1},
    petalStorm:{t:'m',s:'int',f:Math.round(4*magBuff),dc:[1,4],u:14,pst2:1,blc:0.50},
    ancientPower:{t:'m',s:'int',f:Math.round(14*magBuff),dc:[2,6],u:6,ap:1},
    transference:{t:'s',s:'int',f:0,dc:[1,1],u:4,tr:1},
    moonlight:{t:'h',s:'int',f:0,dc:[1,1],u:8,ml:1,hl:1},
    readScripture:{t:'s',s:'int',f:Math.round(6*soulBuff),dc:[1,6],u:14,rs2:1},
  ...
  function clrEff(b){if(b.stn?.p==='rest'&&b.chp>0&&!b.fainted){b.chp=Math.min(b.mhp,b.chp+Math.floor(b.mhp*0.20));}b.dlt=0;b.wb=0;b.sa=false;b.slt=false;b.fl=false;b.swl=false;b.dtt=false;b.ds=0;b.cp=false;b.stats.atk=b.origStats.atk+(b.stn?.sb?.atk||0);if(b.ws>0)b.ws--;rmS(b);}
  ...
  function matchScore(me,them){let s=0;if(me.cls.sw.atk>=3&&them.stats.def<10)s+=15;if(me.cls.sw.int>=3&&them.stats.int<10)s+=15;if(me.cls.id==='paladin'&&them.cls.id==='rogue')s+=20;if(me.cls.id==='mage'&&them.cls.id==='paladin')s+=15;if(me.cls.id==='warrior'&&them.cls.id==='rogue')s+=15;if(me.cls.id==='rogue'&&them.cls.id==='mage')s+=5;if(me.cls.id==='warrior'&&them.cls.id==='druid')s+=15;if(me.cls.id==='druid'&&them.cls.id==='rogue')s+=15;if(me.cls.id==='cleric')s-=10;s+=(me.chp/me.mhp-them.chp/them.mhp)*20;return s;}
  function aiDecision(active,bench,enemy){if(active.swl||active.lookAtMeLocked)return{a:'fight'};const ab2=bench.filter(b=>!b.fainted&&b!==active);if(!ab2.length)return{a:'fight'};const cs=matchScore(active,enemy);let bs2=null,bsc=cs;for(const b of ab2){const sc=matchScore(b,enemy)+(b.chp/b.mhp)*10;if(sc>bsc){bsc=sc;bs2=b;}}if(active.chp<active.mhp*0.25&&bs2&&bsc>cs+10)return{a:'switch',t:bs2};if(bs2&&bsc>cs+25)return{a:'switch',t:bs2};return{a:'fight'};}
  function am(a,d2,myTeam){if(a.chp<a.mhp*0.3){const hi=a.ab.findIndex(x=>x.hl&&!x.ch&&x.cu>0);if(hi>=0)return a.ab[hi];}if(a.ps>=2&&a.chp<a.mhp*0.6){const ri=a.ab.findIndex(x=>x.ro&&x.cu>0);if(ri>=0)return a.ab[ri];}const av=a.ab.filter(x=>{if(x.cu<=0)return false;if(x.rqF&&d2.status!=='f')return false;if(x.gw&&a.wb>=8)return false;if(x.ro)return false;return true;});if(!av.length)return null;const w2=av.map(x=>{if(x.hl&&!x.ch)return a.chp<a.mhp*0.5?30:5;if(x.ch)return a.chp<a.mhp*0.8?15:5;if(x.gdl)return a.dlt>0?0:15;if(x.sw)return a.slt?5:18;if(x.gw)return a.wb===0?22:12;if(x.gg)return!terrain?25:3;if(x.ve)return d2.ps>=4?d2.ps*8:2;if(x.sb)return 12;if(x.ev)return d2.ws<3?18:10;if(x.ap)return 22;if(x.tr)return(a.ps+a.brn+a.bs+a.ws+a.cs)*6+(a.status?15:0);if(x.ml){const hurt=myTeam?myTeam.filter(m=>!m.fainted&&m.chp<m.mhp*0.8).length:0;return hurt>0?hurt*12:a.chp<a.mhp*0.7?20:3;}if(x.rs2)return 12;if(x.ij)return 18;if(x.lam)return 10;let avg=x.f+x.dc[0]*(x.dc[1]+1)/2;if(x.dh)avg*=1.8;if(x.cbo&&a.dtt)avg*=1.75;if(x.rqF)avg*=1.3;if(x.bm)avg*=1.4;return avg;});const t2=w2.reduce((s2,v)=>s2+v,0);let r=Math.random()*t2;for(let i=0;i<av.length;i++){r-=w2[i];if(r<=0)return av[i];}return av[av.length-1];}
  function eh(a,d2,ab,myTeam,enemyTeam){
    ...
    if(ab.ml){const heal=Math.max(8,Math.floor(a.mhp*0.25))*(a.stn?.p==='hb'?1.3:1);a.chp=Math.min(a.mhp,a.chp+Math.floor(heal));a.mlHeal=Math.floor(heal);ab.cu--;rs2();return;}
    if(ab.ij){const loss=Math.max(1,Math.floor(d2.mhp*0.20));d2.chp=Math.max(1,d2.chp-loss);d2.dtt=true;ab.cu--;rs2();return;}
    if(ab.lam){d2.lookAtMeLocked=true;ab.cu--;rs2();return;}
    if(ab.rs2&&Math.random()<0.30){ab={...ab,f:Math.round(14*soulBuff),dc:[2,6]};}
    ...
    for(let h=0;h<(ab.dh?2:1);h++){
      ...
      // Crit
      if(dmg>0&&!ab.hl&&(Math.floor(Math.random()*100)+1+a.stats.atk)>critThreshold){dmg=Math.round(dmg*1.5);}
      // Shield
  ...
  function tk(b,enemy){
    ...
    if(b.stn?.p==='cpt'&&b.chp>0&&enemy&&enemy.chp>0){enemy.cs+=cursePerTurn;}
  ...
  function mlHeal(team,switchIn){for(const m of team){if(m!==switchIn&&m.mlHeal>0){switchIn.chp=Math.min(switchIn.mhp,switchIn.chp+m.mlHeal);m.mlHeal=0;break;}}}

  const N=2000;const classWins={},classPicks={};
  for(const cls of CLS){classWins[cls.l]=0;classPicks[cls.l]=0;}
  for(let i=0;i<N;i++){
    terrain=null;const pool=[...CLS].sort(()=>Math.random()-0.5);
    const t1={members:pool.slice(0,4).map(c=>cr(c)),active:null},t2={members:pool.slice(4,8).map(c=>cr(c)),active:null};
    ...
    for(let turn=0;turn<200;turn++){
      ...
      const d1=aiDecision(a,t1.members,b),d2=aiDecision(b,t2.members,a);
      let am1=null,am2=null,sw1=false,sw2=false;
      if(d1.a==='switch'){if(a.lookAtMeLocked){const shT=b.stn?.p==='hb'?3:2;const shH=b.stn?.p==='hb'?20:15;for(const m of t2.members){if(!m.fainted){m.st2=Math.max(m.st2,shT);m.shp=Math.max(m.shp,shH);}}a.lookAtMeLocked=false;am1=am(a,b,t1.members);}else{clrEff(a);t1.active=d1.t;apS(t1.active,rndS(t1.active));if(t1.active.stn?.p==='fb')b.bs+=2;mlHeal(t1.members,t1.active);sw1=true;}}else{am1=am(a,b,t1.members);}
      if(d2.a==='switch'){if(b.lookAtMeLocked){const shT=a.stn?.p==='hb'?3:2;const shH=a.stn?.p==='hb'?20:15;for(const m of t1.members){if(!m.fainted){m.st2=Math.max(m.st2,shT);m.shp=Math.max(m.shp,shH);}}b.lookAtMeLocked=false;am2=am(b,a,t2.members);}else{clrEff(b);t2.active=d2.t;apS(t2.active,rndS(t2.active));if(t2.active.stn?.p==='fb')t1.active.bs+=2;mlHeal(t2.members,t2.active);sw2=true;}}else{am2=am(b,a,t2.members);}
      ...
      if(sw1&&sw2){tk(fa,fb);tk(fb,fa);}
        ...
        for(const fighter of[...t1.members,...t2.members]){if(!fighter.apq)continue;for(let i=fighter.apq.length-1;i>=0;i--){fighter.apq[i].tl--;if(fighter.apq[i].tl<=0){const ap=fighter.apq.splice(i,1)[0];if(ap.tgt.chp>0&&!ap.tgt.fainted&&!fighter.fainted){const raw=Math.round(14*magBuff)+rollDice(2,6);const m=0.5+0.5*(rollDice(2,6)+fighter.stats.int)/(rollDice(2,6)+ap.tgt.stats.int);const dmg=Math.max(1,Math.round(raw*m));ap.tgt.chp-=dmg;ap.tgt.dtt=true;}}}}
        if(terrain&&terrain.tl>0){terrain.tl--;if(terrain.e==='pb'){for(const x of[fa,fb]){if(x.chp>0)aP(x,1,x===fa?fb:fa);}}if(terrain.e==='vine'){const fm2=ai2>=bi2?fa:fb;if(fm2.chp>0){fm2.chp-=4;fm2.dtt=true;}}if(terrain.tl<=0)terrain=null;}
        tk(fa,fb);tk(fb,fa);
      }
      if(t1.active.fainted){const al=t1.members.filter(m=>!m.fainted);if(!al.length){winner=2;break;}let best=al[0],bs2=-999;for(const b2 of al){const sc=matchScore(b2,t2.active);if(sc>bs2){bs2=sc;best=b2;}}t1.active=best;apS(t1.active,rndS(t1.active));if(t1.active.stn?.p==='fb')t2.active.bs+=2;mlHeal(t1.members,t1.active);}
      if(t2.active.fainted){const al=t2.members.filter(m=>!m.fainted);if(!al.length){winner=1;break;}let best=al[0],bs2=-999;for(const b2 of al){const sc=matchScore(b2,t1.active);if(sc>bs2){bs2=sc;best=b2;}}t2.active=best;apS(t2.active,rndS(t2.active));if(t2.active.stn?.p==='fb')t1.active.bs+=2;mlHeal(t2.members,t2.active);}
    }
    if(winner===0)continue;
    const winners=winner===1?t1.members:t2.members;
    for(const m of winners)classWins[m.cls.l]++;
    for(const m of[...t1.members,...t2.members])classPicks[m.cls.l]++;
  }
  const results={};
  for(const cls of CLS){const p=classPicks[cls.l];results[cls.l]=p>0?(classWins[cls.l]/p*100):0;}
  return results;
}

// Test configs
const configs = [
  {name:'Baseline (current)',         physNerf:1.0,  magBuff:1.0,  soulBuff:1.0,  cursePerTurn:1, critThreshold:100},
  {name:'Phys -10%, Mag +10%',       physNerf:0.90, magBuff:1.10, soulBuff:1.10, cursePerTurn:1, critThreshold:100},
  {name:'Phys -15%, Mag +15%',       physNerf:0.85, magBuff:1.15, soulBuff:1.15, cursePerTurn:1, critThreshold:100},
  {name:'Phys -10%, Mag +10%, 2 curse', physNerf:0.90, magBuff:1.10, soulBuff:1.10, cursePerTurn:2, critThreshold:100},
  {name:'Phys -15%, Mag +15%, 2 curse', physNerf:0.85, magBuff:1.15, soulBuff:1.15, cursePerTurn:2, critThreshold:100},
  {name:'Phys -10%, Mag +15%, 2 curse', physNerf:0.90, magBuff:1.15, soulBuff:1.15, cursePerTurn:2, critThreshold:100},
];
...
for (const cfg of configs) {
  ...
  console.log(`\n--- ${cfg.name} (spread: ${spread.toFixed(1)}%) ---`);
  for (const [cls, wr] of sorted) console.log(`  ${cls.padEnd(10)} ${wr.toFixed(1)}%`);
}
